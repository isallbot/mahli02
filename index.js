...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'BOT0-YOE\n' // GANTI NAMA LU
            + 'ORG:OWNER MAHLI;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6281229247198:+62 896-3347-4471\n' // GANTI NOMOR LU
            + 'END:VCARD'
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	...
	baby.on('connecting', () => {
		start('2', 'isall Connecting...')
	})
	baby.on('open', () => {
		success('2', 'isall Connected')
	})
		...
		case 'textlight':
		    ...
		    break
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata mahli`
...
*/		 
				...
				if (!isRegistered) return reply(nad.noregis())
					if (isLimit(sender)) return reply(nad.limitend(pusname))
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}blackpink mahli`)
					pink = body.slice(11)
					reply('「❗」Hah Blekping :v')
					lol = await getBuffer(`https://vinz.zeks.xyz/api/blackpink?text=${pink}&apikey=apivinz`)
...
*/				
            ...
            case 'creator':
                  ...
                  baby.sendMessage(from, 'Tuh Nomor Owner ku , Sedia buka jasa BUAT BOT Dan SEWA BOT HARGA CP OWNER',MessageType.text, { quoted: mek} )
					break
case 'hartatahta':
if (isBanned) return reply(nad.baned())
if (!isRegistered) return reply(nad.noregis())
